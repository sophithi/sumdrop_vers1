...
class ReportController extends Controller
{
    public function index()
    {
        ...
        $summary = [
            'orders_count' => $baseQuery->count(),
            'today_orders' => Order::whereDate('created_at', $today)->count(),
            'month_orders' => Order::whereBetween('created_at', [$monthStart, Carbon::now()])->count(),
            'total_sales_usd' => $baseQuery->where('currency', 'usd')->sum('total'),
            'total_sales_khr' => $baseQuery->where('currency', 'khr')->sum('total'),
            ...
            'cash_count' => $baseQuery->where('payment_method', 'cash')->count(),
            'mobile_count' => $baseQuery->where('payment_method', 'mobile')->count(),
            'cash_total_usd' => $baseQuery->where('payment_method', 'cash')->where('currency', 'usd')->sum('total'),
            'cash_total_khr' => $baseQuery->where('payment_method', 'cash')->where('currency', 'khr')->sum('total'),
            'mobile_total_usd' => $baseQuery->where('payment_method', 'mobile')->where('currency', 'usd')->sum('total'),
            'mobile_total_khr' => $baseQuery->where('payment_method', 'mobile')->where('currency', 'khr')->sum('total'),
